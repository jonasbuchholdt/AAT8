...
% ------------------------------------------------------------------------
function [population]= gen_step(population,n,fit,surquote,replacement,tlevel,pmut,M)
...
for l=1:m
    [childa,childb]=offspring(population,n,parents(m,1),parents(m,2),M);
    pop_new.(strcat('gene',int2str(sursort(2*l-1))))=childa;
    pop_new.(strcat('gene',int2str(sursort(2*l))))=childb;
end
%population=pop_new;
population=mutator(pop_new,n,pmut,M);
end