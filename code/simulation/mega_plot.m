...
for h=1:(180/ares)
    ...
    IR=eval(strcat('data',int2str((astart+(h-1)*ares)*10),'.ir(1:end/2)'));
    [tf1(:,h),w]=freqz(IR,1,20000,fs);
    IR_NEW(:,h) = IR;
    clearlist={strcat('data',int2str((astart+(h-1)*ares)*10))};
    clear(clearlist{:})
end
[discard index]=min(abs(w-f));
for h=180/ares:(300/ares)
