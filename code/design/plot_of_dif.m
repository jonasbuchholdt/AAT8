...
clear all 
filename = 'House-Fire-Alarm.mp3';
%filename = 'Alarm-alert.mp3';
[ip, fs] = audioread(filename);

% add noise
for k = 1 : length(ip);
ip(k) = ip(k); %+ (0.1*randn(1,1));
end



max_expected_period = round(1/200*fs);
min_expected_period = round(1/100*fs);
frame_len = 300*max_expected_period;
...
for k = 1 : length(ip)/frame_len -1;
    ...
    %use xcorr to determine the local period of the frame, autocorrelation
...
% the mitter plot
    rxxc(find(rxxc < 0)) = 0; %set any negative correlation values to zero
    center_peak_width = find(rxxc(frame_len:end) == 0 ,1); %find first zero after center
    %center of rxx is located at length(frame)+1
    rxxc(frame_len-center_peak_width : frame_len+center_peak_width  ) = min(rxxc);
%     hold on
%     plot(lag, rxx,'g');
%     hold off
    [max_val loc] = max(rxxc);
...
f1 = 8; f2=f1*2; 
x = sin(1*pi*f1*t-pi/2) + sin(2*pi*f2*t);
plot(t, x)
ylabel('Amplitude')
xlabel('Time (seconds)')
