...
The wanted filter is attenuated by \SI{2}{\decibel} such that the designed filter also fit at low frequency. Recalling from \autoref{fig:band_pass_filter}, the filter was attenuated by \SI{2.1}{\decibel} such that the center frequency was at \SI{0}{\decibel} the filter have to be gained by \SI{2.1}{\decibel}. In \autoref{fig:bandstop_filter_eq_bodeplot_finish} the filter is attenuated further with \SI{2}{\decibel} and therefore the front gain which i called $In_Gain$ shall be amplified by \SI{4.1}{\decibel} plus the \SI{6.4}{\decibel} which correspond to the inverting in \autoref{fig:band_pass_filter} The gain in front of the cost filter than have to be \SI{10.5}{\decibel}. The corresponding block diagram for the filter including the gain in front of the filter is as \autoref{fig:bandstop_filter_blockdiagram_gain}
